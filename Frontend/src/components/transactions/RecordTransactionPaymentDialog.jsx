...
export default function RecordTransactionPaymentDialog() {
  const open = useTransactionsStore((s) => s.recordPaymentModalOpen);
  const close = useTransactionsStore((s) => s.closeRecordPaymentModal);
  const targetId = useTransactionsStore((s) => s.recordPaymentTargetId);
  const getTransactionById = useTransactionsStore((s) => s.getTransactionById);
  const recordPayment = useTransactionsStore((s) => s.recordPayment);
  ...
  useEffect(() => {
    if (open && item) {
      setPaymentAmount(item.balanceRaw ? String(item.balanceRaw) : "1555");
      setPaymentDate("Choose Date");
  ...
  const handleSubmit = (e) => {
    ...
    if (item) {
      recordPayment(item.id, {
        ...
        date: paymentDate === "Choose Date" ? "Aug 22, 2026" : paymentDate,
        method,
      });
    }
  };
  ...
  const currentPaymentNum = Number(paymentAmount) || 0;
  const currentPaid = item.paidRaw || 0;
  const totalAmount = (item.amountRaw || 0) + (item.fetRaw || 0);
  const newPaid = currentPaid + currentPaymentNum;
    ...
    <Dialog open={open} onOpenChange={(o) => !o && close()}>
      <DialogContent className="sm:max-w-4xl rounded-2xl p-6 gap-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full">
          {/* Top Summary Block (matching Screenshot 4) */}
          <div className="bg-[#f3f4f8] rounded-sm p-5 flex flex-col gap-3.5 border border-border/60">
            <div className="flex items-center justify-between">
              ...
              <span className="font-montserrat font-bold text-base text-purple">{item.reference}</span>
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-montserrat font-bold text-base text-destructive">
                ${(item.balanceRaw || 0).toLocaleString()}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start w-full">
            ...
            <FormField label="Date" labelClassName={LABEL_CLASS} className="flex-1 min-w-0">
              <DatePicker
                className={FIELD_CLASS}
                value={paymentDate === "Choose Date" ? "" : paymentDate}
                onChange={(val) => setPaymentDate(val || "Choose Date")}
          ...
          {/* New Balance & Status Summary Card (matching Screenshot 4) */}
          <div className="bg-emerald-50 rounded-sm p-4 flex flex-col gap-2 border border-emerald-200 mt-1">
            <div className="flex items-center justify-between">
              <span className="font-montserrat text-base text-muted-foreground">New Balance</span>
              <span className="font-montserrat font-bold text-lg text-foreground">
                ${newBalance.toLocaleString()}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="font-montserrat text-base text-muted-foreground">New Status</span>
