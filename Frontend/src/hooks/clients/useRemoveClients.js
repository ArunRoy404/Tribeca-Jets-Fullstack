...
/**
 * Removes several clients at once. Call with an array of ids.
 *
...
export function useRemoveClients() {
  return useMutation({
    mutationFn: (ids) => clientsService.removeMany(ids),
    onSuccess: (result) => {
      invalidate(queryKeys.clients.all);

      const count = result?.affected ?? 0;
      toastSuccess(`${count} ${count === 1 ? "client" : "clients"} removed`);
    ...
    onError: (error) => toastApiError(error, "Could not remove these clients"),
  });
}
