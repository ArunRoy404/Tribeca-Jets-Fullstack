...
- **Uploads do not survive a deploy — and that now includes real documents.**
  `STORAGE_DRIVER=local` writes to Render's ephemeral filesystem, and every
  redeploy or restart wipes it. This used to mean avatars only. Since 23–25 Sep
  it also means **the tax forms in each broker's document folder (#7), quote
  aircraft photos and itinerary uploads**. The database rows survive on Neon,
  so after a deploy they still list the file and `GET /api/uploads/:id` fails
  to find the bytes. Fine for throwaway test data; **before anyone uploads a
  real 1099, set the `S3_*` variables** (any S3-compatible bucket —
  Cloudflare R2's free tier is enough) and change `STORAGE_DRIVER` to `auto`
  or `s3`.
